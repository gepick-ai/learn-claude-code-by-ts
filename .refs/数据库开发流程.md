...
|------|------|--------------|----------------------------|
| **生成 migration 文件** | 产出可版本化迁移脚本（`migration.sql`） | 否 | `bun run db:generate --name <slug>` |
| **运行 migration** | 执行迁移脚本到目标数据库 | 是 | `bun run migrate`（或 selfhost 的 migration service） |
...
4. 编写或调整使用 `db` 的业务代码。
5. 在目标环境执行 **run migration**（例如 `bun run migrate`），确认结构对齐后再启动应用。
...
| 5 | 结构对齐 | `migrate(db, { migrationsFolder })`（与 Drizzle 1.x 约定相同：扫描 `migration/<YYYYMMDDHHMMSS>_<slug>/migration.sql`，无 `meta/_journal.json`） | **只向前**应用未执行过的迁移；当前实现通过独立入口 `packages/app/src/migrate.ts` 执行（`bun run migrate`） |
...
- **运行 migration**：`bun run migrate`（入口：`packages/app/src/migrate.ts`，内部调用 `migrate(db, { migrationsFolder })`）。
