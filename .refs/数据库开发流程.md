...
| 动作 | 本质 | 是否改数据库 | 对应命令（`@gepick/app`） |
|------|------|--------------|----------------------------|
| **生成 migration 文件** | 产出可版本化迁移脚本（`migration.sql`） | 否 | `bun build:migration -- --name <slug>`（仓库根；向底层 `db:generate` 透传参数） |
| **运行 migration** | 执行迁移脚本到目标数据库 | 是 | 仓库根 `bun run:migration`（等同 `@gepick/app` 的 `migrate:apply`）；已构建产物时用 `bun -F '@gepick/app' migrate`（`dist/migrate.js`）；selfhost 见部署文档 |

一句话：**generate 是“写施工图”，run migration 是“按图施工”。**
...
| 生成迁移 | 产出可版本化的 SQL | `bun build:migration -- --name <slug>`（仓库根；向底层 `db:generate` 透传参数）（见 §8） | 每次结构性变更后生成；**人工审阅** 子目录内 `migration.sql` 再提交 |
...
| 静态检查 | 类型与 schema 一致 | `bun` / TypeScript 检查（如根目录 `typecheck:packages` 脚本） | 避免仅运行时才发现列名错误 |
...
4. 编写或调整使用 `db` 的业务代码。
5. 在目标环境执行 **run migration**（例如 `bun migrate`），确认结构对齐后再启动应用。
...
| 5 | 结构对齐 | `migrate(db, { migrationsFolder })`（与 Drizzle 1.x 约定相同：扫描 `migration/<YYYYMMDDHHMMSS>_<slug>/migration.sql`，无 `meta/_journal.json`） | **只向前**应用未执行过的迁移；当前实现通过独立入口 `packages/app/src/migrate.ts` 执行（`bun migrate`） |
...
- **生成**（在 `gepick` 仓库根）：  
  `bun build:migration -- --name add_session_workspace_id`  
  Drizzle Kit 1.x 会自动带时间戳前缀；无需再传 `--prefix`。
- **运行 migration（源码入口）**：`bun -F '@gepick/app' migrate:apply` 或根目录 `bun run:migration`（同入口 `src/migrate.ts`）。**构建产物入口**：`bun -F '@gepick/app' migrate`（`dist/migrate.js`）。
- **应用启动**：`packages/app/src/index.ts` 只初始化 DB 与服务，不再隐式执行迁移。
