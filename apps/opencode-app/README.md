...
一个精简版的 opencode 服务端示例，基于 Bun + Hono + Drizzle(SQLite)。

## 启动项目

下面步骤按“可直接跑起来”写，默认你当前在仓库根目录 `agent-dev`。
...
```bash
bun run db:opencode:push
```

2) 需要提交版本化迁移时，生成 migration 文件：

```bash
bun run db:opencode:generate
```

3) 验证数据库表是否已创建：

```bash
sqlite3 apps/opencode-app/.data/app.db ".tables"
```
