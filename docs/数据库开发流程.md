...
| 生成迁移 | 产出可版本化的 SQL | `bun run db:opencode generate --name <slug>`（见 §8） | 每次结构性变更后生成；**人工审阅** 子目录内 `migration.sql` 再提交 |
...
| 5 | 结构对齐 | `migrate(db, { migrationsFolder })`（与 opencode 相同：扫描 `migration/<YYYYMMDDHHMMSS>_<slug>/migration.sql`，无 `meta/_journal.json`） | **只向前**应用未执行过的迁移；`apps/opencode-app/src/storage/db.ts` 在 `Database.Client()` 内执行 |
...
## 7. 依赖清单（与 opencode 对齐）

| 依赖 | 版本策略 | 用途 |
|------|----------|------|
| `drizzle-orm` | `1.0.0-beta.16-ea816b6`（与 opencode 同构建） | ORM、运行时访问、`migrate` |
| `drizzle-kit` | 同上 | `generate` 产出 **`migration/<tag>/{migration.sql,snapshot.json}`** |
| Bun 内置 `bun:sqlite` | 随 Bun | SQLite 驱动 |

---

## 8. 迁移目录布局与命令（opencode-app）

与 **opencode** 一致：**每个迁移一个子目录**，目录名为 **`YYYYMMDDHHMMSS_<slug>`**，内含 **`migration.sql`** 与 **`snapshot.json`**（由 `drizzle-kit generate` 生成；**无**旧版根目录 `meta/_journal.json`）。

- **生成**（在 `agent-dev` 仓库根）：  
  `bun run db:opencode generate --name add_session_workspace_id`  
  Drizzle Kit 1.x 会自动带时间戳前缀；无需再传 `--prefix`。
- **运行时**：`migrate(db, { migrationsFolder: "<opencode-app>/migration" })`，见 `apps/opencode-app/src/storage/db.ts`。
