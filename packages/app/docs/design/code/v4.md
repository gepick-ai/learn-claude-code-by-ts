...
**定位**：在 v2/v3 已确定的 **磁盘真相 + 静态预览网关** 之上，将每个 project 的代码工作区从「根目录单文件 HTML / 手写多文件静态站」的自然延伸，升级为 **位于工作区子目录 `client/` 下的、与 monorepo 内 `@gepick/client` 选型对齐的标准 Client 工程**。当前内置脚手架为 **Vite + React + JavaScript（JSX）+ Tailwind**，**不写 TypeScript**：优先跑通 **`vite build` → `client/dist`**；后续如需 TS 再在模板代次上演进。**同一 project 工作区根下**后续可并列其它子目录（如 **`server/`**），本版 **仅规范 `client/`**。**v4 与线上预览衔接的唯一运行时形态**仍是 **构建后的静态 Web 资源**（`index.html` + JS/CSS 分包与静态资源），与过去「最终仍是一组可由网关提供的 HTML/CSS/JS 文件」一致，只是工程化源码与 **`npm run build`（或等价）** 均发生在 **`client/`** 内。**开发服务器预览（dev serve）、反向代理 Vite dev、HMR 等不属于 v4**，留待后续版本单独规划。

---

## 1. 设计目标

1. **工程形态对齐**：在 **v2 约定的 project 工作区根**之下，以子目录 **`client/`** 作为前端应用根；脚手架默认具备：`client/package.json`、`vite.config.js`、`jsconfig.json`（路径别名可选）、`src/**/*.jsx`、`index.html`、`eslint.config.js` 等（见 **`client-dev/templates/default-client`**）。依赖与 **React/Vite/Tailwind 生态** 对齐 §2。**不与**会话宿主 `packages/client` 混淆。
...
|------|----------------|
| 运行时 | `react`、`react-dom` |
| 构建 | **`vite`**（脚本 **`vite build`**，不经 `tsc`） |
| UI / 样式 | `tailwindcss`、`@tailwindcss/vite`、`clsx`、`tailwind-merge` |
| React 插件 | `@vitejs/plugin-react` |
| 图标 | `lucide-react` |
| 状态 | `zustand` |
| 语言 | **JavaScript / JSX**（内置模板）；TypeScript 可为后续模板代次 |
| 质量 | `eslint`、`eslint-plugin-react-hooks`、`prettier`（无 **`typescript-eslint`** 于默认模板） |
...
{absoluteProjectDir}/                 # v2 project 工作区根（如 .projects/{projectId}）
  ...
  client/                             # 【v4】**全部**前端代码与 Vite 工程根（仅此目录）
    package.json
    vite.config.js
    jsconfig.json                     # 可选：IDE 路径别名
    eslint.config.js
    index.html                        # Vite HTML 入口（/`src/main.jsx`）
    src/
      main.jsx
      App.jsx
      …
...
- **脚手架**：建议在 **创建 project** 或 **首次进入 code 工作区** 时，由服务端或受信流程在 **`absoluteProjectDir/client/`** 写入 **最小可构建模板**；其中 **`package.json` 必须来自 §2.1 的基准模板**（与 `@gepick/client` 栈同源），其余文件（`vite.config.js`、`src/**/*.jsx`、`index.html`、`eslint.config.js` 等）由模板包提供。工作区根可保留简短 `README.md`。目标是 **`cd client` → `install` → `build`** 路径稳定、**依赖版本跨 project 可控**。
- **Agent**：在 [system prompt 或等价约定](../../../src/agent/prompt.ts) 中明确 **前端根为 `client/`**、栈名、预览依赖 **`client/dist`**；**在完成对 `client/` 的有效改动后须执行 `npm run build`**（必要时先 `npm install`），否则预览无 **`dist`**；并以 **Vite 工程结构** 写码（业务在 **`client/src/`**，**`client/index.html` 仅薄壳**；**禁止**默认采用「根 `index.html` + 根 `*.js` + CDN」主形态）。不得把用户消息中的路径当磁盘根；bash **`cwd` 为工作区根** 时用 **`cd client && …`**。
...
| 2026-04-28 | **基准模板**退化为 **JS/JSX + `vite build`**（无 TS），降低构建失败率；§2、§3.1、定位同步。 |
| 2026-04-28 | §5 Agent：补充 **改完 `client/` 后须 `npm run build`**，否则无 **`dist`**、预览为空。 |
