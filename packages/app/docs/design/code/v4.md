...
- **Agent**：在 [system prompt 或等价约定](../../../src/agent/prompt.ts) 中明确 **前端根为 `client/`**、栈名、**在 `client/` 下执行的** 构建命令、预览依赖 **`client/dist`**；不得把用户消息中的任意路径当作磁盘根，仍以 v2 **工具与工作目录约束** 为准（执行 npm/pnpm 时 **`cwd` 指向 `…/client`**）。
