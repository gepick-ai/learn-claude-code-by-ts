...
- **存储位置**：monorepo 根目录下的 **`.projects/`**（建议通过环境变量 `APP_PROJECT_PATH` 覆盖绝对路径；未设置时解析为进程工作目录所属的仓库根下的 `.projects`，实现细节由实现约定）。
...
- `workspace runtime`：解析 `APP_PROJECT_PATH` / 默认 `.projects` 根路径；按 `projectId` 互斥。
...
| **`projectsRoot`** | 全局根路径：默认 `<monorepo-root>/.projects`，或由 **`APP_PROJECT_PATH`** 指定绝对路径。 |
...
输入：`projectId`（非空字符串）、可选环境变量 `APP_PROJECT_PATH`。

1. **`projectsRoot`**  
   - 若设置了 `APP_PROJECT_PATH`：取其值，`path.resolve` 为绝对路径。  
...
- **`.projectsRoot` 解析**：默认 `<monorepo-root>/.projects`；可通过 `APP_PROJECT_PATH` 指向自定义绝对路径（部署到非 git 工作树时使用）。
...
packages/app/src/
  server/
    project/
      ...
      projects-root.ts                   # 解析 APP_PROJECT_PATH / 默认 .projects
  agent/
