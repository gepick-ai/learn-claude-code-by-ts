# 与 gepick-app（`@gepick/app`）配套对接说明（定稿）

**范围**：本包在业务上如何与 **`packages/app`**（npm 包 **`@gepick/app`**）协同——联调、HTTP/SSE、会话与消息数据流、本地存储、部署衔接；**不重复** 定义 Vite/TS/React/Tailwind/ESLint/Prettier 等纯工程项（见 **[项目工程技术选型](./项目工程技术选型.md)**）。

**适用路径**：`packages/client`  
**配套后端**：`@gepick/app`（Hono），路由含 `/session`、`/sse` 等。
...
| **与现网关系** | 目标为承接 `@gepick/app` 内 `src/server/frontend.html` 的前端能力；**相对路径**、**`localStorage` 键**与现网对齐以便迁移。 |
...
| 项 | 定稿 |
|----|------|
| **开发代理** | Vite `server.proxy` 将 **`/session`、`/sse`（及同前缀）** 转发到**本机正在运行的** `@gepick/app` 的 **origin**。**不**约定固定端口；在 `vite.config` 中从 **`.env` / `.env.local`** 读取（如 **`GEPICK_APP_ORIGIN`**）。 |
...
- 只存 **`sessionId`**，键名 **`gepick-app.sessionId`**（实现会自旧键 `opencode-app.sessionId` 迁移一次）。  
...
| 主题 | 定稿 |
|------|------|
| **SSOT** | 权威数据在 `@gepick/app` + 持久化层。前端 TypeScript 类型与 `packages/app/src/server/session/model.ts` 的 **`SessionMessage` / `Part`** 镜像对齐，不另建冲突 DTO。 |
...
| 集成 | 由 `@gepick/app` 的 `app.ts` 在「仅 `frontend.html`」基础上扩展：托管本包 `index.html` 与静态资源，并保留现有 `/session`、`/sse`；**路径前缀**前后端对齐。 |
...
已确认与 **`@gepick/app` 的对接**：Vite 代理到本机后端、**同域/等效同域**、**`gepick-app.sessionId`**、上述 API 与数据流、**Zustand** 在会话/SSE 中的角色、Hono 托管方案方向；不首版支持跨域 SSE + cookie 组合。
...
| 2026-04-24 | 产品/包名与目录对齐为 gepick；应用与 SDK 位于 `packages/app`、`packages/client`、`packages/sdk`；教程为仓库根目录 `tutorial/`（`@gepick/tutorial`）；环境变量 `GEPICK_APP_ORIGIN` 等。 |
