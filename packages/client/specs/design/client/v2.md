...
| 日期 | 说明 |
|------|------|
| 2026-04-28 | 从原《项目工程技术选型》中拆出业务/后端与数据流、Zustand 产品内用法；工程项保留在《项目工程技术选型》。 |
| 2026-04-28 | 产品/包名与目录对齐为 gepick；应用与 SDK 位于 `packages/app`、`packages/client`、`packages/sdk`；教程为仓库根目录 `tutorial/`（`@gepick/tutorial`）；环境变量 `GEPICK_APP_ORIGIN` 等。 |
