...
import { MemberManager, memberManager, messageBus, MessageBus, TeamProtocol, WORKDIR } from "./tools";
...
const TOOL_HANDLERS = new Map<string, (input: unknown) => Promise<string>>([
    ...
    ["get_shutdown_status", (input) => TeamProtocol.getShutdownStatusFromInput(input)],
    ["send_shutdown_request", (input) => TeamProtocol.sendShutdownRequestFromInput(input)],
    ["send_plan_reply", (input) => TeamProtocol.sendPlanReplyFromInput(input)],
]);
...
const TOOLS: Anthropic.Tool[] = [
    ...
    TeamProtocol.GetShutdownStatus,
    TeamProtocol.SendShutdownRequest,
    TeamProtocol.SendPlanReply
];
