...
export type FileIconProps = React.SVGAttributes<SVGSVGElement> & {
...
export function FileIcon(props: FileIconProps) {
  const { node, className, expanded, mono, ...rest } = props
  const name = useMemo(() => chooseIconName(node.path, node.type, expanded || false), [expanded, node.path, node.type])
  const id = `file-icon-mono-${useId()}`
  return (
    <svg data-component="file-icon" {...rest} className={clsx(className)}>
      {mono ? (
        <>
          <defs>
            <mask id={id} style={{ maskType: "alpha" } as CSSProperties}>
